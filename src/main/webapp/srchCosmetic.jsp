	...
	<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
		...
		<div class="container">
				<%
					...
					ArrayList<Cosmetic> searchList = cosmeticDAO.getSrchList(srchKey, srchText, pageNumber);
					int cnt = cosmeticDAO.getSrchCount(srchKey, srchText) - ((pageNumber - 1) * 10);
					for (int i=0; i<searchList.size(); i++) {
				%>
				
					<h3><strong>검색 결과</strong></h3>
					<br>
					<a href="<%=searchList.get(i).getCosmeticKind()%>1.jsp?cosmeticID=<%= searchList.get(i).getCosmeticID() %>"><img src=<%=searchList.get(i).getCosmeticImage()%> width="200" height="200"></a>
					<td><a href="<%=searchList.get(i).getCosmeticKind()%>1.jsp?cosmeticID=<%= searchList.get(i).getCosmeticID() %>">
					<h5><%= searchList.get(i).getCosmeticName().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br>") %></h5>
